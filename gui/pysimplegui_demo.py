...
def main():
    ...
    window = sg.Window('Demo Application - OpenCV Integration', layout, no_titlebar=False, location=(0, 0), resizable=True)
...
#
#     # create the window and show it without the plot
#     window = sg.Window('Demo Application - OpenCV Integration', layout, resizable=True, no_titlebar=False, location=(0, 0), finalize=True)
#
#     # locate the elements we'll be updating. Does the search only 1 time
#     image_elem = window['-IMAGE-']
#     slider_elem = window['-SLIDER-']
#     timeout = 1000 // fps  # time in ms to use for window reads
#
#     # Calculate space between the window and the image element itself. Will be used to resize element as window changes
#     image_offset = (window.size[0] - image_elem.get_size()[0], window.size[1] - image_elem.get_size()[1])
#     # Grab window size inorder to correctly respond to window resize events
...
#
#         imgbytes = cv.imencode('.ppm', frame)[1].tobytes()  # can also use png.  ppm found to be more efficient
#         image_elem.update(data=imgbytes, size=(window.size[0] - image_offset[0], window.size[1] - image_offset[1]))
#
#
# main()
#
